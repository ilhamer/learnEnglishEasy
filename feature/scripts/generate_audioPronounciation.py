...
def generate_english_audio():
    ...
    INPUT_JSON = os.path.join(BASE_DIR, "data", "pronunciation.json")
    OUTPUT_DIR = os.path.join(BASE_DIR, "audio", "pronunciation")
    ...
    try:
        ...
        for item in data:
            ...
            if "audio_file" not in item:
                item["audio_file"] = os.path.join("audio", "pronunciation", audio_filename).replace("\\", "/")
